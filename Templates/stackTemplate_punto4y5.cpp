...
int main() {
  ...
  // Se procede a instanciar con un diferente tipo de dato, en este caso un string
  Stack<std::string> s_string;

  // Se envian los strings a la pila
  s_string.push("Esto");
  s_string.push("Es una");
  s_string.push("Prueba");

  // El tamaño de la pila se imprime en pantalla
  std::cout << "Stack size: " << s_string.size() << std::endl;

  // Uno de los valores se imprime 
  s_string.foreach([](std::string& value) {
    std::cout << "Value: " << value << std::endl;
  });

  // Vacia la pila y lo muestra en consola
  try {
    while (!s_string.empty()) {
      std::string value = s_string.pop();
      std::cout << "Popped value: " << value << std::endl;
    }

    // Tamaño final de la pila luego de limpiarla
    std::cout << "Stack size: " << s_string.size() << std::endl;
  } catch (const std::exception& e) {
    std::cerr << "Exception: " << e.what() << std::endl;
  }
